  ...
  fileFilter: function (req, file, next) {
    ...
    if (image) {
      ...
      next({ message: "File type not suported" });
            ...
            // console.log(
            //   // User.find({})
            //   // .then(users=>console.log(users.map(x=>x.username))),
  ...
  User.findById(req.params.id, (err, user) => {
    if (err) {
      ...
      User.find({ username: req.body.username }, (err, i) => {
        ...
        User.findByIdAndUpdate(
          req.params.id,
          { friends: user.friends, requests: request },
          (err, x) => {
            User.find({ username: req.body.username }, (err, i) => {
              if (i[0].friends) {
                i[0].friends = [
                  ...i[0].friends,
                  { username: x.username, image: x.photo },
                ];
              }
              var requested = i[0].requested.filter((x) => x != user.username);
              User.findByIdAndUpdate(
                i[0]._id,
                { friends: i[0].friends, requested: requested },
                (err, j) => {
                  res.json(j);
                }
              );
  ...
  User.findById(req.params.id, async (err, found) => {
    if (err) {
      ...
      }
      {
        ...
        let documents = {
          documents: [
            { id: req.params.id, language: "en", text: req.body.description },
          ],
        };
        ...
        axios({
          method: "post",
          url:
            "https://westcentralus.api.cognitive.microsoft.com/text/analytics/v2.1/sentiment?showStats=false",
...
router.post(
  "/:id/coverphoto",
  multer(multerConf).single("photo1"),
  (req, res) => {
